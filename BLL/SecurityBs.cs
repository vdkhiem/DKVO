...
namespace BLL
{
    ...
    public class LinkHubMembershipProvider : MembershipProvider
    {
        ...
        {
            int count = db.GetAll().Where(p => p.UserEmail == username && p.Password == password).Count();
            return (count != 0);
        }
    }

    /// <summary>
    /// Customize Role Provider
    /// </summary>
    public class LinkHubRoleProvider : RoleProvider
    {
        ...
        {
            string[] s = {db.GetAll().Where(p=>p.UserEmail == username).FirstOrDefault().Role} ;
            return s;
        }
